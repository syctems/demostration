...
(function(q) {
	...
	(function(a) {
		...
		a.Chart.prototype.getDataRows = function(f) {
			...
			var x, y;
			n = f ? [D, v] : [v];
			for (w = B.length; w--;) x = B[w][0], y = B[w][1], d = l[x], e.sort(function(b, a) {
		...
		a.Chart.prototype.getTable = function(a) {
			...
			function(a, d, c) {
				...
				if (b) {
					for (f += "\x3ctr\x3e"; e < c; ++e) b = a[e], k = a[e + 1], b === k ? ++g : g ? (f += q("th", "highcharts-table-topheading", 'scope\x3d"col" colspan\x3d"' + (g + 1) + '"', b), g = 0) : (b === d[e] ? h.exporting.useRowspanHeaders ? (k = 2, delete d[e]) : (k = 1, d[e] = "") : k = 1, f += q("th", "highcharts-table-topheading", 'scope\x3d"col"' + (1 < k ? ' valign\x3d"top" rowspan\x3d"' + k + '"' : ""), b));
					f += "\x3c/tr\x3e"
				}
				if (d) {
					f += "\x3ctr\x3e";
